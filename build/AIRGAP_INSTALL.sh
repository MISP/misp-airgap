...
# This Script installs MISP on Ubuntu 20.04 without DB so it can be used for installation inside a container 
...
MISPvars () {
    ...
    MISP_LIVE="1" # Change later? -> only set true in prod
...
installDeps () {
  ...
  [[ -n $UNATTENDED ]] && sudo DEBIAN_FRONTEND=noninteractive apt install -qy postfix || sudo apt install -qy postfix
